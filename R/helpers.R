...
save_tasktab = function(tasks, path = here::here("tables", "tasktab.csv")) {
  ...
  tasktab[, cph_misspecified := ifelse(zph_pval_processed < 0.05 | is.na(zph_pval_processed), 1, 0)]
  tasktab[, noverp := n / p]
...
#' Create tasks and corresponding instantiated resamplings from dataset files
#'
#' Reads `.rds` dataset files from `./datasets/`, creates `TaskSurv` objects,
#' and either recreates resamplings from stored CSV files or instantiates new ones.
#'
#' @param conf `config::get()` configuration object.
#' @return A list with elements `tasks` (named list of `TaskSurv`) and `resamplings`
#'   (named list of instantiated `Resampling` objects).
create_tasks_and_resamplings = function(conf) {
  set.seed(conf$seed)
    ...
    if (fs::file_exists(resampling_csv)) {
      ...
      if (conf$outer_eval$resampling == "repeated_cv") {
        if (conf$outer_eval$repeats == "auto") {
          ...
          repeats = assign_repeats(num_events)

          cli::cli_alert_info("Adjusting number of repeats: {.val {num_events}} events -> {.val {repeats}} repeats")
        } else {
          cli::cli_alert_info("Using fixed number of repeats: {.val {repeats}}")
        }
      }
  ...
  list(tasks = tasks, resamplings = resamplings)
}
...
#' @param conf `config::get()` for result paths.
#' @param verbose (`logical(1)`) Diligently report file writes. Default `FALSE`.
...
#' a backup location.
#' @param conf `config::get()` for registry paths.
#' @param tmp_path Path to move duplicate archives to. Default `here::here("tmp", "archive-backup")`.
#'
#' @example
