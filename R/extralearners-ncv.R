# Since we can't update mlr3extrealearners without also updating mlr3 and related packages due to bumped dependencies,
# for reproducibility's sake we need to manually add the new learner definitions.
# THe alternative would be to partially rewrite the experiment (due to breaking changes in mlr3, mlr3tuning, ...)
# and also update several learners, which would most definitely require rerunning the whole benchmark... which is very expensive.
require(mlr3extralearners)
require(paradox)

# Extracted learner definition from https://github.com/mlr-org/mlr3extralearners/pull/536
...
LearnerSurvNCVsurv = R6::R6Class(
  ...
  private = list(
    ...
    .predict = function(task) {
      ...
      # Changed from original learner as surv_return is the current name in proba
      # and it wasn't exported before
