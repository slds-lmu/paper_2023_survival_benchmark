...
- Datasets are loaded from `./datasets` in `.rds` format, where they are also stored in CSV and arff formats.
  - Code for retrieval and minimal preprocessing is in `scripts/import_datasets.R`
...
- `scripts/process-results.R` processes the raw `batchtools` registry into aggregated result files.
...
The dataset's names, source package, and OpenML dataset IDs are stored in `./tables/dataset_table.[csv|rds]`.
...
dataset_tbl = readRDS(here::here("tables", "dataset_table.rds"))
