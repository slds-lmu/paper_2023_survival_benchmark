...
- Holdout outer resampling
- Holdout inner resampling
- 1 eval random search

So 2 learner `$train()` and and `$predict()` calls total.
...
plot_resources <- function(xdf, by = "task_id", measure = "runtime", ...) {
  ...
  if (measure == "runtime") {
    p <- p + scale_x_time(breaks = (3600 * 24) * 1:20, minor_breaks = 3600 * 12 * 1:20)
  }
  if (measure == "mem.used") {
...
```{r}
alljobs[findRunning(), c("learner_id", "task_id", "tags", "submitted")]  |>
