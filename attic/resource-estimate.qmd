...
# if (!(file.exists("bmr-resource-est.rds"))) {
#   bmr = reduceResultsBatchmark(store_backends = FALSE)
#   saveRDS(bmr, "bmr-resource-est.rds")
# }
```
...
total_estimate <- runtimes |>
  dplyr::filter(measure %in% c("harrell_c", "rcll")) |>
...
```{r}
# resource_est_tab <- alljobs |>
#   dplyr::mutate(
#     mem.used = ifelse(is.na(mem.used), max(mem.used, na.rm = TRUE), mem.used),
#     hours = runtime/2/60/60,
#     hours = ifelse(type == "estimated", max(hours), hours),
#     mem_gb = mem.used / 1000
#   ) |>
#   dplyr::select(
#     learner_id, task_id,
#     n, p, n_uniq_t,
...
write.csv(resource_est_tab, file = here::here("attic", "resource_est.csv"), row.names = FALSE)
```
