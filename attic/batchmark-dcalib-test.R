...
names = c("aids2", "lung", "metabric", "veteran")
files_sub = fs::path_ext_remove(fs::path_file(files))
files = files[which(files_sub %in% names)]
...
for (i in seq_along(files)) {
  ...
  if (isTRUE(all.equal(c(outer_folds, inner_folds), c(1, 1)))) {
    folds = 1
