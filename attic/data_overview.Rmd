...
# All datasets currently in datasets with pkg::dataset notation, using
...
# Do files in datasets and files in above table match up?
setdiff(
  datasets$dataset,
  fs::dir_ls(here::here("datasets"), glob = "*.rds") |> fs::path_file() |> fs::path_ext_remove()
)
...
xdat_summary <- fs::dir_ls(here::here("datasets"), glob = "*.rds") |>
  purrr::map_df(~{
    dat <- readRDS(.x)
    ...
    tibble(
      ...
      n_uniq_t = length(unique(dat$time)),
...
write.csv(datasets, here::here("attic", "datasets.csv"), row.names = FALSE)

datasets |>
  # arrange(tolower(dataset)) |>
  arrange(desc(n_uniq_t)) |>
