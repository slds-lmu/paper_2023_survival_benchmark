...
job_milestones = job_milestones |>
  dplyr::mutate(
    duration = round(difftime(last, first, units = "days"), 2),
    days_since =  round(difftime(Sys.time(), last,units = "days"), 2)
  )
```
...
job_completion |>
  filter(done >= lubridate::now() - lubridate::ddays(7)) |>
  ggplot(aes(x = done, y = prop)) +
  geom_step(linewidth = 1.5) +
  ...
  scale_x_datetime(
    date_breaks = "1 days", date_labels = "%b %d"
