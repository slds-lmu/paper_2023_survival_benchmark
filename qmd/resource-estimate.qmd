...
- Only run jobs tuend on `harrell_c` and untuned learners (plus CoxBoost)
- Budget:
  - 5 + n_params evaluations for MBO
  - 3-fold CV for tuning (1 iter)

- Etxrapolating outer eval should be straight-forward (tmes 3 for CV, times `n_repeats` as needed)
...
tab = collect_job_table(reg, resource_est_file = "")
...
tab[,
  time.running.hms := hms::hms(seconds = as.numeric(runtime, units = "secs"))
]
...
runtimes |>
  dplyr::mutate(
    time.running.hms = hms::round_hms(time.running.hms, secs = 1)
  ) |>
...
plot_resources(tab, by = "tags", measure = "runtime")
summarize_resources(tab, by = "tags", measure = "runtime")
```
...
plot_resources(tab, by = "tags", measure = "mem.used")
summarize_resources(tab, by = "tags", measure = "mem.used")
```
...
write.csv(
  resource_est_tab,
  file = here::here("attic", "resource_est_dec.csv"),
  row.names = FALSE
)
```
