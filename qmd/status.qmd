...
```{r status-by-group}
check_job_state(tab, byvars = "time_cat") |>
...
```{r status-by-measure}
check_job_state(tab, byvars = "measure") |>
  tablify()
```
...
```{r status-by-learner}
check_job_state(tab, byvars = "learner_id") |>
  tablify()
```
...
```{r status-by-task}
check_job_state(tab, byvars = "task_id") |>
