...
- `aggr` are aggregates scores, i.e., one score per `task` and `learner` and `tuning_measure`, derived from `scores`
...
| [`scores.csv`][scores_csv]            | Same scores, but in CSV format for portability |
