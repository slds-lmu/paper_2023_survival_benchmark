...
Violation of the proportional hazards (PH) assumption is derived from the p-value of the global significance test conducted with `cox.zph()` at the 5% level (i.e., `cph_misspecified = p.value > 0.05`).
...
tasktab |>
  ...
  dplyr::mutate(
    ...
    cph_misspecified = factor(ifelse(cph_misspecified == 1, "Yes", "No")),
    noverp = round(noverp, 2)
  ) |>
  dplyr::select(task_id, repeats, n, p, noverp, events, censprop, n_uniq_t, cph_misspecified) |>
  setNames(c(
    "Dataset",
    "# CV Repeats",
    "N",
