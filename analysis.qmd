...
bma_harrell_c = remove_results(bma_harrell_c, learner_id_exclude = "SSVM", task_id_exclude = "patient")
bma_rcll      = remove_results(bma_rcll,      learner_id_exclude = "SSVM", task_id_exclude = "patient")
...
```{r}
bma1 = copy(bma_harrell_c$data)
bma1[, tuned := "harrell_c"]

bma2 = copy(bma_rcll$data)
bma2[, tuned := "rcll"]
...
bma = bma |>
  dplyr::mutate(
    learner_group = dplyr::case_match(learner_id,
      c("KM", "NA", "AK") ~ "Baseline",
...
saveRDS(bma, here::here("results/registry_beartooth/bma_full.rds"))
```
...
- Task `patient` is excluded as it turned out to be simulated and having 100% administrative censoring
...
#| fig-width: 10
for (measure_id in measures_eval_ids) {
  if (measure_id %in% c("caliba_ratio")) next
  if (msr_tbl[id == measure_id, erv]) next
  plot_results(bma = bma_harrell_c, type = "cd_n", measure_id = measure_id, tuning_measure_id = "harrell_c", ratio = 1)
}
```
...
for (measure_id in measures_eval_ids) {
  if (measure_id %in% c("caliba_ratio")) next
  if (msr_tbl[id == measure_id, erv]) next
  plot_results(bma = bma_rcll, type = "cd_n", measure_id = measure_id, tuning_measure_id = "rcll", ratio = 1)
}
```
...
for (measure_id in measures_eval_ids) {
  if (measure_id %in% c("caliba_ratio")) next
  if (msr_tbl[id == measure_id, erv]) next
  plot_results(bma = bma_harrell_c, type = "cd_bd", measure_id = measure_id, tuning_measure_id = "harrell_c", ratio = 1, baseline = "CPH")
}
```
...
for (measure_id in measures_eval_ids) {
  if (measure_id %in% c("caliba_ratio")) next
  if (msr_tbl[id == measure_id, erv]) next
  plot_results(bma = bma_rcll, type = "cd_bd", measure_id = measure_id, tuning_measure_id = "rcll", ratio = 1, baseline = "CPH")
}
```
...
```{r}
ggplot(bma, aes(x = learner_id, y = rcll, color = tuned, fill = tuned)) +
